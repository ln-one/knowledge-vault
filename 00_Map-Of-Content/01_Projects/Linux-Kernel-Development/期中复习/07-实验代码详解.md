...
| 参数 | 名称 | 位数 | 说明 | 示例 |
|------|------|------|------|------|
| %1 | 基址 | 32位 | 段在内存中的起始地址 | 0x00100000 |
| %2 | 界限 | 20位 | 段的大小（如果G=1，单位是4KB） | 0xFFFFF |
| %3 | 属性 | 12位 | 段类型、DPL、粒度等 | DA_C + DA_32 |
...
; 这个描述符的含义：
; - 基址：0（从0地址开始）
; - 界限：0xFFFFF（如果G=1，实际大小是4GB）
; - 属性：DA_C（代码段）+ DA_32（32位）
```
...
| 参数 | 名称 | 说明 | 示例 |
|------|------|------|------|
| %1 | 目标段选择子 | 指向目标代码段 | SelectorCode |
| %2 | 目标偏移 | 目标代码段内的偏移地址 | 0x1000 |
| %3 | 参数个数 | 栈切换时复制的参数数量(0-31) | 2 |
| %4 | 属性 | 门类型和DPL | DA_386CGate + DA_DPL3 |
...
; 计算方式：
; - LABEL_DESC_CODE_RING3 - LABEL_GDT: 描述符在GDT中的偏移
; - + SA_RPL3: 加上RPL=3（请求特权级为Ring 3）
```
